...
def network_mask():
    ...
    ### topography
    ds_topo = xr.open_dataset(f'{root_dir}/../ESM/other_file/GEBCO_2014_1x1_global.nc')
    ...
    # land-sea mask
    # land=0, sea=1
    ds_lsmask = xr.open_dataset(f'{root_dir}/../ESM/other_file/lsmask.nc').sortby('lat').squeeze().drop('time')
...
def calc_seasonal(array_2d, N_time=133, period=12):
    nan_mask = ~(np.all(np.isnan(array_2d), axis=0))
